...
#1) No se puede saber que paso mal porque el except no indica qué tipo de error ocurrió, entonces se pierde toda la información.
#2) Solo se imprimiría "Ocurrió un error". No se sabría cual fue el problema del código.
#3) Para la depuracion es muy dificil, porque no muestra los errores reales y evita detectar los fallos.
...
#1) FileNotFoundError si el archivo no existe.
#ValueError si algún dato no es numérico.
#ZeroDivisionError si se divide entre 0.
#PermissionError si no se puede abrir el archivo.
#2) No, ya que, en cada tipo de error se necesita realizar una acción diferente.
#3) El error, la causa y la línea donde ocurrió.
...
#1) El usuario es notificado con un mensaje de error y ahí si intentar guardar de nuevo.
#2) Mostrando un mensaje, algo así como "Error al guardar".
#3) Sí, porque si el guardado falla, el sistema debe avisar del problema.
...
#1) Primero (else) es para ejecutar solamente si no hay ningún error en el codigo y (finally) se ejecuta siempre sin importar de
#que haya un error.
#2) (Else) lo usaría para ejecutar el código que depende del éxito del try y (finally) lo usaría para limpiar, cerrar archivos
#y así.
#3) El finally se ejecuta antes de que (return) se devuelva, luego imprime “Se ejecuta igual” y luego devuelve 1.
...
#RESPUESTAS DEL PROBLEMA 5.
#1) A mi parecer pienso que para la validación de datos sería el ValueError.
#2) El que valor fallo, el porque y lo que se buscaba.
#3) Le ayuda a saber el error y le muestra mensajes para que pida de nuevo lo que necesite y lo lleve acabo correctamente.
...
#RESPUESTAS DEL PROBLEMA 6.
#1) Cuando uno pueda resolver el problema de forma segura por si mismo.
#2) Cuando se necesita registrar información para la depuración.
#3) Cuando no sabes que hace con el error y si llega a pasar eso hay que dejar que lo revise para que el sistema lo registre
# correctamente.
...
#RESPUESTAS DEL PROBLEMA 7.
#1) Puede ser, porque si todo es dependiente de todo a la vez, pues si llega a haber un error en cualquier lado va a dar error,
# en cambio si los elementos son independientes puede que no de error a todo.
#2) Guardando los errores en una lista para luego revisarlos y notificarlos.
#3) Pasaria que se notificaría que no hubo ningun resultado válido, y revisar los datos de entrada del porque esta todo mal.
